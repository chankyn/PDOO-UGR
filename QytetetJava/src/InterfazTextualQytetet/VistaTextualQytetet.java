...
public int menuGestionInmobiliaria(){ //ejemplo de men�
    ...
    public int menuSalirCarcel(){
        ...
        menu.put(0, "Pagar la libertad");
        menu.put(1, "Tirar el dado");
        return seleccionMenu(menu);
    }
  
    public boolean elegirQuieroComprar(){
        Map<Integer,String> menu = new TreeMap();
        menu.put(0, "Comprar propiedad.");
        menu.put(1, "No comprar propiedad.");
        if (seleccionMenu(menu)==0)
            return true;
        else 
            return false;
    }
...
//M�todo para controlar la elecci�n correcta de una opción en el men� que recibe como argumento   
{   boolean valido = true; 
    int numero;
    String lectura;
    do { // Hasta que se hace una selecci�n v�lida
      for(Map.Entry<Integer, String> fila : menu.entrySet()) {
            ...
            this.mostrar(numero + " : " + texto);  // n�mero de opción y texto
      }
      this.mostrar("\n Elige una opción: ");
